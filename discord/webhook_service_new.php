...
class WebhookServiceNew {
    ...
    public function isAuthorizedForWebhook($webhookId) {
        try {
            ...
            error_log('WebhookServiceNew - Authorization check failed: ' . $e->getMessage());
            return false;
        }
    }
    ...
    public function getWebhookById($webhookId) {
        try {
            ...
            error_log('WebhookServiceNew - Error getting webhook: ' . $e->getMessage());
            return false;
        }
    }
    ...
    public function addWebhook($webhookUrl, $webhookName, $channelName, $description = '') {
        try {
            ...
            error_log('WebhookServiceNew - Error adding webhook: ' . $e->getMessage());
            return [
                'status' => 'error',
    ...
    public function importSharedWebhook($sharingCode) {
        try {
            ...
            error_log('WebhookServiceNew - Error importing webhook: ' . $e->getMessage());
            return [
                'status' => 'error',
    ...
    public function updateWebhook($webhookId, $webhookName, $channelName, $description = '') {
        try {
            ...
            error_log('WebhookServiceNew - Error updating webhook: ' . $e->getMessage());
            return [
                'status' => 'error',
    ...
    public function deleteWebhook($webhookId) {
        try {
            ...
            error_log('WebhookServiceNew - Error deleting webhook: ' . $e->getMessage());
            return [
                'status' => 'error',
    ...
    public function setDefaultWebhook($webhookId) {
        try {
            ...
            error_log('WebhookServiceNew - Error setting default webhook: ' . $e->getMessage());
            return [
                'status' => 'error',
    ...
    private function ensureDefaultWebhook() {
        try {
            ...
            error_log('WebhookServiceNew - Error ensuring default webhook: ' . $e->getMessage());
            return false;
        }
    }
    ...
    public function sendTestMessage($webhookId) {
        try {
            ...
            error_log('WebhookServiceNew - Error sending test message: ' . $e->getMessage());
            return [
                'status' => 'error',
    ...
    private function logWebhookUsage($webhookId, $generatorType, $contentSummary, $statusCode, $isSuccess, $errorMessage = null) {
        try {
            ...
            error_log('WebhookServiceNew - Error logging webhook usage: ' . $e->getMessage());
            return false;
        }
    }
}
